...
async fn on_stripped_state_member(
    ...
    if room_member.state_key != ctx.client.user_id().await.unwrap() {
        return;
    }

    if let Room::Invited(room) = ctx.room {
...
async fn login_and_sync(
    ...
    client.register_event_handler(on_stripped_state_member);
